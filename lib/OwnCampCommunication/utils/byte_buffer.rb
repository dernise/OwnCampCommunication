...
class ByteBuffer
  ...
  def order=(endianness)
    @order = endianness
    @short_pack_order = endianness == :little_endian ? 'v' : 'n'
  ...
  def put_short(i, offset=nil)
    return if i > 65535
    a=[]
    [i].pack(@short_pack_order).each_byte { |b| a << b }
    put_array(a, offset)
  end
  ...
  def put_cstring(val)
    put_array(to_utf8(val.to_s).unpack('C*') + [0])
  end
  ...
  def get_short
    check_read_length(2)
    vals = ""
    (@cursor..@cursor+1).each { |i| vals << @buf[i].chr }
    @cursor += 2
    vals.unpack(@short_pack_order)[0]
  end
